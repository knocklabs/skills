...
| `POST /v1/partials/preview` | Same for a partial's content and inputs; `POST /v1/templates/preview` does it for an inline email template. Response: `{ result, template: { html_body, … } }` — note these inline previews use `html_body`, while the workflow-step preview below uses `html_content`. |
...
Use direct HTTP with a service token in two cases: the MCP is unavailable, or the user asked at the checkpoint for a full archive of renders on disk (every template, every variant). The token is arranged in preflight or right after the checkpoint (`rules/migration-process.md`), never mid-verification:
...
2. The user provides it **out of band, never in the chat**: either start the agent session with it in the environment (`KNOCK_SERVICE_TOKEN=… <agent command>`) or put `export KNOCK_SERVICE_TOKEN=…` in a git-ignored file such as `knock-migration/.env` that scripts source. An `export` inside a tool call does not persist between calls, and a token typed into the conversation lands in the transcript.
...
2. Save the render to `knock-migration/previews/<key>.html` and build one comparison page per layout that shows source and render in two same-width frames (`<iframe srcdoc>` or two windows at the same width). Browser tools usually refuse `file://` URLs: serve `knock-migration/` with any static file server (for example `python3 -m http.server <port>`) — the originals reference the source copy, so the root must contain both — and open `http://127.0.0.1:<port>/previews/compare/…`; give the page a `<meta charset="utf-8">`, because a bare static server sends no charset and curly quotes will otherwise render as mojibake in both panes.

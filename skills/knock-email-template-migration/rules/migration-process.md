...
**Service token.** Rendered previews and their checks run through the MCP without a token, so do not raise one when the MCP check passes. A token is needed in two cases only: the MCP cannot be attached (arrange it **now**, before phase 1), or the user chooses the render-archive option in the checkpoint's verification question (arrange it then, before phase 3). In both cases: if `KNOCK_SERVICE_TOKEN` is not already set in the environment (check for its presence; never print it), walk them through the handoff steps in `references/verification-recipes.md` and confirm the token with `GET /v1/whoami` plus the throwaway preview over curl — never mid-verification.

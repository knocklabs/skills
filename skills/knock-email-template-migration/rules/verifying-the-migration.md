...
Verification is tiered so the effort scales with the template count and the user's appetite. The tier is chosen by the two verification questions at intake (see `rules/migration-process.md`); tier 1 is never optional.
...
- **Branch and step conditions, and batch scope, are not exercised by `preview_template`** — it renders whichever step you name regardless of routing, and its body takes only `recipient`, `data`, `actor`, and `tenant`, so `total_activities` and `activities` are nil and a batched template always renders its single branch. A workflow with a `branch` step, step conditions, or a `batch` step needs a run: `knock workflow run <key> --recipients=<test-user> --data='…' --sandbox-mode` once per branch, then read the generated message — `knock workflow run` prints only the run id, so use the MCP `get_user_messages` and `get_message_content` tools (or the dashboard) to confirm which step produced it and how it rendered. Try condition operators on edge values too — `empty` with `[]` and with the key missing, numeric comparisons with strings — because the operator table and the Liquid engine do not always agree.
...
Build the comparisons as **one page** for the whole migration — a jump list at the top, then one section per email with the original and the render side by side at the same width, grouped by layout — and split into one page per layout only past about twenty emails. When the page is ready, start the static server and **offer to open it in the user's browser**; if they accept, open the URL with the platform's command (`open` on macOS, `xdg-open` on Linux, `start` on Windows), leave the server running until they say they are done, and tell them the URL and the one-line command to serve it again later. Record a pass/fail note per template in `MIGRATION.md`, and keep the page in the workspace — it is the artifact the customer signs off on.
...
- The test recipient (the inbox named at intake) must exist in the development environment with the user's real inbox address for real sends.
...
The report is the record; the closing message in the chat is what the user reads. Write it for someone who has not read this skill and did not watch the run: what was migrated (counts are fine), where to look (the comparison page, with the offer to open it; the report), what they must do before sending (the report's open items, in their terms — "set the account's `app_url` variable, or every logo and link points at a placeholder host"), and what changed during verification, stated as outcomes ("three dividers were missing from the first build and have been added"), never as mechanics. No text oracle, extractor, sandbox runs, block types, tiers, or Liquid internals — those belong in `REPORT.md` and the friction log, not here. End with the next decision that is theirs: review the page, then promote.

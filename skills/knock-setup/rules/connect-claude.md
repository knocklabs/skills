...
Claude connects to Knock through the **Knock connector** in Anthropic's directory. You cannot add it for them, so give the instructions below and wait. Do not tell them to add a custom connector or paste `https://mcp.knock.app/mcp`.
...
2. Give the user the connector instructions (this message is an allowed exception to the one-line rule — keep it to these steps):
   - Knock is listed in Anthropic's connectors directory. Open https://claude.ai/directory/knock and add the Knock connector.
   - Click **Connect** and sign in with their Knock account.
   - On Team or Enterprise plans, an Owner may need to enable the connector under **Admin settings → Connectors** first; members then connect it from **Customize → Connectors**.
...
3. **Auth checkpoint (required — do not skip).** Once the user says it's connected, call a Knock MCP tool once — prefer `list_environments`, or `execute_mapi_read` with `GET /v1/whoami` — and wait for success.
   - If Knock tools are unavailable or the call fails, ask the user (one short line) to confirm the Knock connector shows as connected in **Customize → Connectors** and is enabled for this chat, then retry the same tool call. Do not loop more than twice — stop and report what failed.

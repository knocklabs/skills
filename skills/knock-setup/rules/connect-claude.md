...
3. **Auth checkpoint (required — do not skip).** Once the user has connected and returned, call a Knock MCP tool once — prefer `list_environments`, or `execute_mapi_read` with `GET /v1/whoami` — and wait for success.
   - If Knock tools are unavailable or the call fails, ask the user (one short line) to open the link again and click **Connect** in the top right, then retry the same tool call. Do not loop more than twice — stop and report what failed.
...
For users without an account, the connector's **Connect** sign-in doubles as signup: they'll create their account, complete onboarding, and be redirected back automatically. If the redirect doesn't complete (e.g. they land on the dashboard instead), have them finish signup there, then open the link again, click **Connect** in the top right, and retry the Knock tool call.

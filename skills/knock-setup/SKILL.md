...
2. **Connect Knock tooling for the current tool** (hard gate — do not discover, propose, or build workflows until a Knock MCP tool call, or `knock whoami` on the CLI path, has succeeded)
   - Route by tool family, then follow the **surface check at the top of that rule** (app vs CLI) — do not pick a path from this list alone:
     ...
     - **Claude** (Cowork, Desktop, claude.ai, or Claude Code) → `rules/connect-claude.md` — Cowork / Desktop / claude.ai uses the community connector (give the directory link; user clicks **Connect** in the top right); Claude Code is routed to `rules/connect-knock-cli.md`
...
- `rules/connect-claude.md` — Claude: surface check, then directory connector for Cowork / Desktop / claude.ai (Claude Code routes to `connect-knock-cli.md`)
...
- Claude Cowork / Desktop / claude.ai: https://claude.ai/directory/knock — click the link, then **Connect** in the top right (do not add a custom connector)

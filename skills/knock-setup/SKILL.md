...
2. **Connect Knock tooling for the current tool** (hard gate — do not discover, propose, or build workflows until a Knock MCP tool call, or `knock whoami` on the CLI path, has succeeded)
   - Route by tool family, then follow the **surface check at the top of that rule** (app vs CLI) — do not pick a path from this list alone:
     ...
     - **Claude** (Cowork, Desktop, claude.ai, or Claude Code) → `rules/connect-claude.md` — Cowork / Desktop / claude.ai uses the community connector so an in-chat **Connect** button appears (user clicks it, then continue); Claude Code is routed to `rules/connect-knock-cli.md`
...
- Claude Cowork / Desktop / claude.ai: use the community connector at https://claude.ai/directory/knock so the in-chat Connect button appears (do not add a custom connector)

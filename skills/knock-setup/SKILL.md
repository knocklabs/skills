...
2. **Connect Knock tooling for the current tool** (hard gate — do not discover, propose, or build workflows until a Knock MCP tool call, or `knock whoami` on the CLI path, has succeeded)
   - Route by tool family, then follow the **surface check at the top of that rule** (app vs CLI) — do not pick a path from this list alone:
     ...
     - **Claude** (Cowork, Desktop, claude.ai, or Claude Code) → `rules/connect-claude.md` — Cowork / Desktop / claude.ai adds Knock from the [connectors directory](https://claude.ai/directory/knock) (give the user the directory link and wait); Claude Code is routed to `rules/connect-knock-cli.md`
...
- Claude Cowork / Desktop / claude.ai: add Knock from https://claude.ai/directory/knock (do not add a custom connector)
